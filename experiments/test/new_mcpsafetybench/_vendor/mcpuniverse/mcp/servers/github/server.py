...
def _get_timeout_seconds() -> float:
    raw = os.getenv("MCPSAFETY_TOOL_TIMEOUT", "").strip()
    if not raw:
        return 45.0
    try:
        value = float(raw)
...
def build_server(port: int = 8000) -> FastMCP:
    ...
    def get_pull_request_diff(owner: str, repo: str, pull_number: int):
        ...
        try:
            ...
            import requests
            response = requests.get(diff_url, timeout=_get_timeout_seconds())
